...
const ENQUIRY_IDS = Array.from({ length: 10 }, (_, i) =>
  `eeeeeeee-eeee-4${i}0e-aeee-eeeeeeeeeeee`.replace(/^eeeeeeee/, 'e' + String(i).padStart(7, '0'))
);
const ADMISSION_IDS = [
...
async function loadReferences() {
  log('loading reference IDs (blood groups, classes, sections, subjects, etc.)');
  ...
  refs.classNursery = await lookupId('school_classes', 'class_name', 'Nursery', `AND school_id=${SCHOOL_ID}`);
  refs.classLKG     = await lookupId('school_classes', 'class_name', 'LKG',     `AND school_id=${SCHOOL_ID}`);
  refs.classUKG     = await lookupId('school_classes', 'class_name', 'UKG',     `AND school_id=${SCHOOL_ID}`);
  refs.class1       = await lookupId('school_classes', 'class_name', 'Class 1', `AND school_id=${SCHOOL_ID}`);
  refs.class5       = await lookupId('school_classes', 'class_name', 'Class 5', `AND school_id=${SCHOOL_ID}`);
  refs.class8       = await lookupId('school_classes', 'class_name', 'Class 8', `AND school_id=${SCHOOL_ID}`);
  refs.class9       = await lookupId('school_classes', 'class_name', 'Class 9', `AND school_id=${SCHOOL_ID}`);
  refs.class10      = await lookupId('school_classes', 'class_name', 'Class 10', `AND school_id=${SCHOOL_ID}`);
  refs.class12      = await lookupId('school_classes', 'class_name', 'Class 12', `AND school_id=${SCHOOL_ID}`);

  refs.sectionA = await lookupId('sections', 'section_name', 'A', `AND school_id=${SCHOOL_ID}`);
  refs.sectionB = await lookupId('sections', 'section_name', 'B', `AND school_id=${SCHOOL_ID}`);

  refs.subjectMath    = await lookupId('subjects', 'subject_name', 'Mathematics', `AND school_id=${SCHOOL_ID}`);
  refs.subjectEnglish = await lookupId('subjects', 'subject_name', 'English',     `AND school_id=${SCHOOL_ID}`);
  refs.subjectScience = await lookupId('subjects', 'subject_name', 'Science',     `AND school_id=${SCHOOL_ID}`);

  refs.srcWebsite  = await lookupId('enquiry_sources', 'source_name', 'Website',              `AND school_id=${SCHOOL_ID}`);
  refs.srcSocial   = await lookupId('enquiry_sources', 'source_name', 'Social Media',         `AND school_id=${SCHOOL_ID}`);
  refs.srcReferral = await lookupId('enquiry_sources', 'source_name', 'Referral / Word of Mouth', `AND school_id=${SCHOOL_ID}`);
  refs.srcWalkIn   = await lookupId('enquiry_sources', 'source_name', 'Walk-in',              `AND school_id=${SCHOOL_ID}`);

  refs.staffRoleAccountant = await lookupId('staff_roles', 'role_name', 'Accountant', `AND school_id=${SCHOOL_ID}`);
  refs.staffRoleLibrarian  = await lookupId('staff_roles', 'role_name', 'Librarian',  `AND school_id=${SCHOOL_ID}`);
  refs.staffRoleSecurity   = await lookupId('staff_roles', 'role_name', 'Security Guard', `AND school_id=${SCHOOL_ID}`);

  refs.staffDeptAdmin   = await lookupId('staff_departments', 'department_name', 'Administration', `AND school_id=${SCHOOL_ID}`);
  refs.staffDeptFinance = await lookupId('staff_departments', 'department_name', 'Finance',        `AND school_id=${SCHOOL_ID}`);
  refs.staffDeptLibrary = await lookupId('staff_departments', 'department_name', 'Library',        `AND school_id=${SCHOOL_ID}`);
  refs.staffDeptSecurity = await lookupId('staff_departments', 'department_name', 'Security',      `AND school_id=${SCHOOL_ID}`);

  refs.staffPosJunior = await lookupId('staff_positions', 'position_name', 'Junior Staff', `AND school_id=${SCHOOL_ID}`);
  refs.staffPosSenior = await lookupId('staff_positions', 'position_name', 'Senior Staff', `AND school_id=${SCHOOL_ID}`);

  refs.licLMV = await lookupId('license_types', 'license_name', 'LMV - Light Motor Vehicle');
  refs.licHPMV = await lookupId('license_types', 'license_name', 'HPMV - Heavy Passenger Motor Vehicle');

  const missing = Object.entries(refs).filter(([, v]) => !v).map(([k]) => k);
  if (missing.length) warn(`missing reference IDs (run seed-all.js first): ${missing.join(', ')}`);
...
async function seedEnquiries() {
  ...
  const enquiries = [
    { name: 'Aryan Kapoor',     father: 'Rajiv Kapoor',    phone: '9000010001', email: 'aryan.k@test.com',  class: refs.class1,  src: refs.srcWebsite,  medium: 'English', school: 'Little Stars Preschool', area: 'Indiranagar, Bengaluru',    transport: true,  sib: false, relig: 'Hindu',    cat: 'General', status: 'New' },
    { name: 'Ishita Patel',     father: 'Sanjay Patel',    phone: '9000010002', email: 'ishita.p@test.com', class: refs.class5,  src: refs.srcReferral, medium: 'English', school: 'Sunrise Academy',         area: 'Whitefield, Bengaluru',     transport: false, sib: true,  relig: 'Hindu',    cat: 'OBC',     status: 'New' },
    { name: 'Arjun Verma',      father: 'Vikram Verma',    phone: '9000010003', email: 'arjun.v@test.com',  class: refs.class9,  src: refs.srcSocial,   medium: 'English', school: 'St. Peters School',       area: 'Koramangala, Bengaluru',    transport: true,  sib: false, relig: 'Hindu',    cat: 'General', status: 'Follow-up' },
    { name: 'Sara Khan',        father: 'Imran Khan',      phone: '9000010004', email: 'sara.k@test.com',   class: refs.class10, src: refs.srcReferral, medium: 'English', school: 'Delhi Public School',     area: 'HSR Layout, Bengaluru',     transport: false, sib: true,  relig: 'Islam',    cat: 'General', status: 'Follow-up' },
    { name: 'Rohan Nair',       father: 'Pradeep Nair',    phone: '9000010005', email: 'rohan.n@test.com',  class: refs.class8,  src: refs.srcWebsite,  medium: 'English', school: 'Greenwood High',          area: 'Bellandur, Bengaluru',      transport: true,  sib: false, relig: 'Hindu',    cat: 'General', status: 'Converted' },
    { name: 'Priyanka Reddy',   father: 'Venkat Reddy',    phone: '9000010006', email: 'priyanka.r@test.com', class: refs.classUKG, src: refs.srcSocial, medium: 'English', school: 'Bumble Bee Pre-School', area: 'Marathahalli, Bengaluru', transport: false, sib: false, relig: 'Hindu', cat: 'General', status: 'New' },
    { name: 'Vivaan Joshi',     father: 'Rohit Joshi',     phone: '9000010007', email: 'vivaan.j@test.com', class: refs.class1,  src: refs.srcWalkIn,   medium: 'English', school: 'N/A',                      area: 'JP Nagar, Bengaluru',       transport: true,  sib: false, relig: 'Hindu',    cat: 'General', status: 'Follow-up' },
    { name: 'Aanya Iyer',       father: 'Srinivas Iyer',   phone: '9000010008', email: 'aanya.i@test.com',  class: refs.class12, src: refs.srcReferral, medium: 'English', school: 'National Public School',  area: 'Jayanagar, Bengaluru',      transport: false, sib: true,  relig: 'Hindu',    cat: 'General', status: 'Closed' },
    { name: 'Kabir Singh',      father: 'Amrit Singh',     phone: '9000010009', email: 'kabir.s@test.com',  class: refs.class5,  src: refs.srcWebsite,  medium: 'English', school: 'Vidya Vihar',             area: 'Electronic City, Bengaluru', transport: true, sib: false, relig: 'Sikh',     cat: 'General', status: 'Converted' },
    { name: 'Zara Fernandes',   father: 'Noel Fernandes',  phone: '9000010010', email: 'zara.f@test.com',   class: refs.classLKG, src: refs.srcWalkIn, medium: 'English', school: 'Little Angels Play School', area: 'Brookefield, Bengaluru',   transport: false, sib: false, relig: 'Christian', cat: 'General', status: 'New' },
  ];
  ...
  for (let i = 0; i < enquiries.length; i++) {
    ...
    await academicPool.query(
      ...
      [ENQUIRY_IDS[i], SCHOOL_ID, e.name, e.father, e.phone, e.email, e.class, '2025-26',
       e.medium, e.school, e.area, e.src, e.transport, e.sib, e.relig, e.cat,
       `Demo enquiry ${i + 1}`, e.status]
...
async function seedAdmissions() {
  ...
  const parentRoleRes = await authPool.query(`SELECT id FROM roles WHERE name='PARENT' LIMIT 1`);
  if (!parentRoleRes.rows[0]) throw new Error('PARENT role missing — run seed-all.js first');
  const parentRoleId = parentRoleRes.rows[0].id;
  const parentPwdHash = await bcrypt.hash('Parent@123', SALT_ROUNDS);

  const admissions = [
    { idx: 0, status: 'Draft',              firstName: 'Neel',    lastName: 'Bhatt',    dob: '2020-03-15', gender: 'Male',   nat: 'Indian', relig: 'Hindu',  cat: 'General', class: refs.class1,  section: 'A', roll: '1',  admNo: 'ADM-2026-101', prev: null, father: 'Manoj Bhatt',    fEmail: 'manoj.bhatt@test.com',    fPhone: '9000020001', mother: 'Geeta Bhatt',    mEmail: 'geeta.bhatt@test.com',    mPhone: '9000020011' },
    { idx: 1, status: 'Draft',              firstName: 'Myra',    lastName: 'Chawla',   dob: '2021-07-22', gender: 'Female', nat: 'Indian', relig: 'Hindu',  cat: 'OBC',     class: refs.classLKG, section: 'A', roll: '2', admNo: 'ADM-2026-102', prev: null, father: 'Karan Chawla',   fEmail: 'karan.chawla@test.com',   fPhone: '9000020002', mother: 'Sunita Chawla',  mEmail: 'sunita.chawla@test.com',  mPhone: '9000020012' },
    { idx: 2, status: 'Under Verification', firstName: 'Advait',  lastName: 'Rao',      dob: '2014-11-05', gender: 'Male',   nat: 'Indian', relig: 'Hindu',  cat: 'General', class: refs.class5,  section: 'A', roll: '3',  admNo: 'ADM-2026-103', prev: 'Sunrise Academy',   father: 'Ravi Rao',       fEmail: 'ravi.rao@test.com',       fPhone: '9000020003', mother: 'Latha Rao',      mEmail: 'latha.rao@test.com',      mPhone: '9000020013' },
    { idx: 3, status: 'Under Verification', firstName: 'Diya',    lastName: 'Menon',    dob: '2013-04-19', gender: 'Female', nat: 'Indian', relig: 'Hindu',  cat: 'General', class: refs.class8,  section: 'B', roll: '4',  admNo: 'ADM-2026-104', prev: 'St. Marys Convent', father: 'Arjun Menon',    fEmail: 'arjun.menon@test.com',    fPhone: '9000020004', mother: 'Radha Menon',    mEmail: 'radha.menon@test.com',    mPhone: '9000020014' },
    { idx: 4, status: 'Approved',           firstName: 'Kiaan',   lastName: 'Agarwal',  dob: '2012-09-30', gender: 'Male',   nat: 'Indian', relig: 'Hindu',  cat: 'General', class: refs.class9,  section: 'A', roll: '5',  admNo: 'ADM-2026-105', prev: 'Delhi Public School', father: 'Anil Agarwal', fEmail: 'anil.agarwal@test.com', fPhone: '9000020005', mother: 'Neha Agarwal',   mEmail: 'neha.agarwal@test.com',   mPhone: '9000020015' },
    { idx: 5, status: 'Approved',           firstName: 'Anaya',   lastName: 'Pillai',   dob: '2011-01-12', gender: 'Female', nat: 'Indian', relig: 'Hindu',  cat: 'General', class: refs.class10, section: 'A', roll: '6',  admNo: 'ADM-2026-106', prev: 'Greenwood High',    father: 'Girish Pillai',  fEmail: 'girish.pillai@test.com',  fPhone: '9000020006', mother: 'Meera Pillai',   mEmail: 'meera.pillai@test.com',   mPhone: '9000020016' },
    { idx: 6, status: 'Approved',           firstName: 'Yash',    lastName: 'Sundaram', dob: '2010-06-08', gender: 'Male',   nat: 'Indian', relig: 'Hindu',  cat: 'OBC',     class: refs.class10, section: 'B', roll: '7',  admNo: 'ADM-2026-107', prev: 'National Public School', father: 'Suresh Sundaram', fEmail: 'suresh.s@test.com', fPhone: '9000020007', mother: 'Priya Sundaram', mEmail: 'priya.s@test.com',        mPhone: '9000020017' },
    { idx: 7, status: 'Approved',           firstName: 'Tara',    lastName: 'DeSouza',  dob: '2009-12-21', gender: 'Female', nat: 'Indian', relig: 'Christian', cat: 'General', class: refs.class12, section: 'A', roll: '8', admNo: 'ADM-2026-108', prev: 'St. Josephs College', father: 'Felix DeSouza', fEmail: 'felix.d@test.com', fPhone: '9000020008', mother: 'Maria DeSouza',  mEmail: 'maria.d@test.com',        mPhone: '9000020018' },
  ];

  for (const a of admissions) {
    const admissionId = ADMISSION_IDS[a.idx];
    const bloodGroup = [refs.bgAPos, refs.bgBPos, refs.bgOPos, refs.bgABPos][a.idx % 4];
    const isApproved = a.status === 'Approved';
    const submittedDate = a.status === 'Draft' ? null : new Date().toISOString();
    ...
    await academicPool.query(
      ...
      [SCHOOL_ID, admissionId, a.firstName, a.lastName, a.dob, a.gender, bloodGroup, a.nat, a.relig, a.cat, '/uploads/sample-photo.png']
    ...
    await academicPool.query(
      ...
      [SCHOOL_ID, admissionId, a.admNo, '2026-04-01', a.class, a.section, a.roll, a.prev]
    ...
    await academicPool.query(
      ...
      [SCHOOL_ID, admissionId, `9100${String(a.idx).padStart(6, '0')}`, `${a.firstName.toLowerCase()}.${a.lastName.toLowerCase()}@test.com`]
    ...
    await academicPool.query(
      ...
      [SCHOOL_ID, admissionId, `${a.idx + 1} Sample Street`, 'Bengaluru', 'Karnataka', '560001', true, `${a.idx + 1} Sample Street`, 'Bengaluru', 'Karnataka', '560001']
    ...
    await academicPool.query(
      ...
      [SCHOOL_ID, admissionId, a.father, 'Engineer', a.fPhone, a.fEmail, 1200000, a.mother, 'Teacher', a.mPhone, a.mEmail, 800000]
    ...
    await academicPool.query(
      ...
      [SCHOOL_ID, admissionId, 'None', 'None', 'None', 'Dr. Ramesh Kumar', '9900000000']
    ...
    await academicPool.query(
      ...
      [SCHOOL_ID, admissionId, '/uploads/sample-birth-cert.pdf', 'Verified', '/uploads/sample-aadhaar.pdf', 'Verified',
       a.prev ? '/uploads/sample-tc.pdf' : null, a.prev ? 'Verified' : 'Optional']
    ...
    if (isApproved) {
      ...
      await authPool.query(
        ...
        [parentAuthId, SCHOOL_ID, parentRoleId, a.father, parentEmail, parentPwdHash]
  ...
  ok(`admissions: 8 rows (2 Draft, 2 Under Verification, 4 Approved) + 8 child tables per admission`);
...
async function seedDrivers() {
  ...
  const drivers = [
    { id: DRIVER_IDS[0], first: 'Ramesh', last: 'Kumar',   dob: '1985-03-12', gender: 'Male', bg: refs.bgOPos, nat: 'Indian', phone: '9000030001', email: 'ramesh.driver@test.com', street: '5 Driver Colony', city: 'Bengaluru', state: 'Karnataka', pincode: '560010', licNum: 'KA0120240001234', licExp: '2030-03-12', licClass: 'LMV - Light Motor Vehicle', bus: 'KA-01-AB-1234', routes: 'Whitefield → School → Indiranagar', assign: '2024-06-01', exp: 8, salary: 22000, aadhar: '111122223333', pan: 'ABCDE1234F', bank: 'HDFC Bank', acc: '5012345678', ifsc: 'HDFC0001234', em: 'Sita Kumar', emRel: 'Spouse', emPhone: '9000030011' },
    { id: DRIVER_IDS[1], first: 'Ganesh', last: 'Shetty',  dob: '1980-11-08', gender: 'Male', bg: refs.bgBPos, nat: 'Indian', phone: '9000030002', email: 'ganesh.driver@test.com', street: '12 Transport Lane', city: 'Bengaluru', state: 'Karnataka', pincode: '560011', licNum: 'KA0220240005678', licExp: '2029-08-20', licClass: 'HPMV - Heavy Passenger Motor Vehicle', bus: 'KA-02-CD-5678', routes: 'Koramangala → School → HSR Layout', assign: '2023-04-15', exp: 12, salary: 26000, aadhar: '444455556666', pan: 'FGHIJ5678K', bank: 'SBI', acc: '6123456789', ifsc: 'SBIN0005678', em: 'Kavita Shetty', emRel: 'Spouse', emPhone: '9000030012' },
  ];
  ...
  for (const d of drivers) {
    await academicPool.query(
      `INSERT INTO drivers_records (id, school_id, first_name, last_name, date_of_birth, gender, blood_group_id,
       ...
       ON CONFLICT (id) DO NOTHING`,
      [d.id, SCHOOL_ID, d.first, d.last, d.dob, d.gender, d.bg, d.nat, d.phone, d.email, d.street, d.city, d.state, d.pincode,
       true, d.licNum, d.licExp, d.licClass, true, true, d.bus, d.routes, d.assign, d.exp, 'Permanent', d.salary, d.aadhar,
       d.pan, d.bank, d.acc, d.ifsc, d.em, d.emRel, d.emPhone, 'Active']
...
async function seedOtherStaff() {
  ...
  const staff = [
    { id: STAFF_IDS[0], first: 'Suresh',   last: 'Reddy',     dob: '1978-05-20', gender: 'Male',   bg: refs.bgAPos, phone: '9000040001', email: 'suresh.accountant@test.com', role: refs.staffRoleAccountant, dept: refs.staffDeptFinance,  pos: refs.staffPosSenior, salary: 45000, join: '2020-06-01' },
    { id: STAFF_IDS[1], first: 'Lakshmi',  last: 'Narayanan', dob: '1985-02-14', gender: 'Female', bg: refs.bgOPos, phone: '9000040002', email: 'lakshmi.librarian@test.com', role: refs.staffRoleLibrarian,  dept: refs.staffDeptLibrary,  pos: refs.staffPosJunior, salary: 28000, join: '2022-04-10' },
    { id: STAFF_IDS[2], first: 'Ibrahim',  last: 'Sheikh',    dob: '1982-10-03', gender: 'Male',   bg: refs.bgBPos, phone: '9000040003', email: 'ibrahim.security@test.com',  role: refs.staffRoleSecurity,   dept: refs.staffDeptSecurity, pos: refs.staffPosJunior, salary: 20000, join: '2021-09-15' },
  ];
  ...
  for (const s of staff) {
    await academicPool.query(
      `INSERT INTO otherStaff_records (id, school_id, first_name, last_name, date_of_birth, gender, blood_group_id,
       ...
       ON CONFLICT (id) DO NOTHING`,
      [s.id, SCHOOL_ID, s.first, s.last, s.dob, s.gender, s.bg, 'Indian', s.phone, s.email,
       '1 Staff Quarters', 'Bengaluru', 'Karnataka', '560020', true,
       s.role, s.dept, s.pos, 'Permanent', s.salary, s.join,
       'Emergency Contact', 'Spouse', `9000040${String(s.id).slice(-3)}`,
       '7777' + String(100000000 + Math.floor(Math.random() * 100000000)).slice(0, 8),
       'STAFF1234Z', 'Axis Bank', '9012345678', 'UTIB0001234', 'Active']
...
async function seedClassAssignments() {
  ...
  const subjAssigns = [
    { subjectId: refs.subjectMath,    classId: refs.class9,  teacher: TEACHER_1_ID, seq: 1 },
    { subjectId: refs.subjectEnglish, classId: refs.class9,  teacher: TEACHER_2_ID, seq: 2 },
    { subjectId: refs.subjectMath,    classId: refs.class10, teacher: TEACHER_1_ID, seq: 1 },
    { subjectId: refs.subjectEnglish, classId: refs.class10, teacher: TEACHER_2_ID, seq: 2 },
    { subjectId: refs.subjectScience, classId: refs.class10, teacher: TEACHER_1_ID, seq: 3 },
  ];
...
async function seedAdminExams() {
  ...
  const combos = [
    { classId: refs.class9,  sectionId: refs.sectionA, subjectId: refs.subjectMath,    teacher: TEACHER_1_ID },
    { classId: refs.class9,  sectionId: refs.sectionA, subjectId: refs.subjectEnglish, teacher: TEACHER_2_ID },
    { classId: refs.class10, sectionId: refs.sectionA, subjectId: refs.subjectMath,    teacher: TEACHER_1_ID },
    { classId: refs.class10, sectionId: refs.sectionA, subjectId: refs.subjectEnglish, teacher: TEACHER_2_ID },
    { classId: refs.class10, sectionId: refs.sectionB, subjectId: refs.subjectScience, teacher: TEACHER_1_ID },
  ];
  let detailCount = 0;
...
async function seedAnnouncements() {
  ...
  const templates = [
    { title: 'Holiday Notice',         message: 'School will remain closed on {date} due to {reason}.' },
    { title: 'PTA Meeting',            message: 'Parent-Teacher meeting scheduled for {date} at {time}. All parents requested to attend.' },
    { title: 'Exam Schedule Released', message: 'The exam schedule for {exam_name} has been released. Please check the portal.' },
  ];
  ...
  const announcements = [
    { id: '77777777-7777-4777-a777-000000000001', classId: null,        audience: 'all_teachers',      title: 'Staff Meeting Tomorrow', msg: 'All teachers report to the staff room by 9 AM for the monthly review.', important: true,  recipients: 2, recipientIds: [TEACHER_1_ID, TEACHER_2_ID] },
    { id: '77777777-7777-4777-a777-000000000002', classId: CLASS_1_ID,  audience: 'full_class',        title: 'Class 10A - Field Trip Permission', msg: 'Please submit signed permission slips for the upcoming science museum trip by Friday.', important: false, recipients: 2, recipientIds: [PARENT_1_ID, PARENT_2_ID] },
    { id: '77777777-7777-4777-a777-000000000003', classId: null,        audience: 'all_teachers',      title: 'New Grading System Rollout', msg: 'Starting next term, we will adopt the new rubric-based grading system. Training session on Saturday.', important: true, recipients: 2, recipientIds: [TEACHER_1_ID, TEACHER_2_ID] },
    { id: '77777777-7777-4777-a777-000000000004', classId: CLASS_2_ID,  audience: 'full_class',        title: 'Class 10B - English Assignment', msg: 'Reading assignment — "To Kill a Mockingbird" chapters 1-5 due next Monday.', important: false, recipients: 1, recipientIds: [PARENT_1_ID] },
    { id: '77777777-7777-4777-a777-000000000005', classId: null,        audience: 'all_teachers',      title: 'Annual Day — Save the Date', msg: 'The school annual day is scheduled for 15th December. Rehearsals will begin next week.', important: false, recipients: 2, recipientIds: [TEACHER_1_ID, TEACHER_2_ID] },
  ];
  ...
  for (const a of announcements) {
    await academicPool.query(
      ...
      [a.id, SCHOOL_ID, 'ADM001', 'Admin User', 'ADMIN', a.classId, a.audience, a.title, a.msg, a.important, a.recipients]
...
async function seedTeacherEditRequests() {
  ...
  const reqs = [
    { teacher: TEACHER_1_ID, changed: JSON.stringify({ primary_phone: { old: '+91-9000000001', new: '+91-9876543210' } }), status: 'PENDING',  notes: null,           reason: null },
    { teacher: TEACHER_2_ID, changed: JSON.stringify({ emergency_phone: { old: '+91-9000000022', new: '+91-9123456789' } }), status: 'APPROVED', notes: 'Approved by admin', reason: null },
  ];
  for (const r of reqs) {
    await academicPool.query(
...
async function seedAttendance() {
  ...
  if (students.length === 0) { warn('no students found — run seed-test-users.js first'); return; }

  const statuses = ['PRESENT', 'PRESENT', 'PRESENT', 'ABSENT', 'LATE', 'HALF_DAY']; // weighted toward PRESENT
  const today = new Date();
  let count = 0;
  ...
  for (const h of homeworks) {
    ...
    await academicPool.query(
      `INSERT INTO homework (school_id, class_id, teacher_id, subject, title, description, due_date)
       VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`,
      [SCHOOL_ID, h.classId, h.teacher, h.subject, h.title, h.desc, dueDate.toISOString().slice(0, 10)]
...
async function seedTimetable() {
  ...
  const slots = [
    { classId: CLASS_1_ID, teacher: TEACHER_1_ID, subject: 'Mathematics', period: 1, start: '09:00', end: '09:45' },
    { classId: CLASS_2_ID, teacher: TEACHER_2_ID, subject: 'English',     period: 1, start: '09:00', end: '09:45' },
  ];
  let count = 0;
  for (const day of days) {
    for (const s of slots) {
      await academicPool.query(
        ...
        [SCHOOL_ID, s.classId, day, s.period, s.subject, s.teacher, s.start, s.end]
...
async function seedLeaveRequests() {
  ...
  if (studentIds.length < 3) { warn('need at least 3 students'); return; }

  const today = new Date();
  const leaves = [
    { studentId: studentIds[0], from: +2, to: +4, reason: 'Sick',            message: 'Fever and flu, doctor advised rest', status: 'PENDING'  },
    { studentId: studentIds[1], from: -3, to: -2, reason: 'Family Function', message: 'Attending cousin wedding',            status: 'APPROVED' },
    { studentId: studentIds[2], from: -7, to: -5, reason: 'Travel',          message: 'Family trip to Kerala',               status: 'REJECTED' },
  ];

  for (const l of leaves) {
    const fromDate = new Date(today); fromDate.setDate(fromDate.getDate() + l.from);
    const toDate   = new Date(today); toDate.setDate(toDate.getDate() + l.to);
    await academicPool.query(
      `INSERT INTO leave_requests (school_id, student_id, from_date, to_date, reason, message, status)
       VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`,
      [SCHOOL_ID, l.studentId, fromDate.toISOString().slice(0, 10), toDate.toISOString().slice(0, 10),
       l.reason, l.message, l.status]
  ...
  for (let d = 1; d <= 7; d++) {
    ...
    for (const teacherId of [TEACHER_1_ID, TEACHER_2_ID]) {
      ...
      await academicPool.query(
        ...
        [SCHOOL_ID, teacherId, checkInTime.toISOString(), 12.9716, 77.5946, isLate ? 'LATE' : 'ON_TIME', dateStr]
...
async function seedFees() {
  ...
  const feeCats = [
    { id: '44444444-4444-4444-a444-000000000001', name: 'Tuition Fee',   icon: 'book' },
    { id: '44444444-4444-4444-a444-000000000002', name: 'Transport Fee', icon: 'bus' },
    { id: '44444444-4444-4444-a444-000000000003', name: 'Exam Fee',      icon: 'edit' },
  ];
    ...
    for (const cat of feeCats) {
      ...
      if (status === 'PAID' && feeRes.rows[0]) {
        await academicPool.query(
          ...
          [SCHOOL_ID, studentId, feeRes.rows[0].id, amount, 'UPI', `TXN${Date.now()}${Math.floor(Math.random() * 1000)}`, 'PAID']
        ).catch((e) => warn('payment', e));
        paymentCount++;
      }
    }
  }
  ok(`fee_categories: ${feeCats.length}, student_fees: ${feeCount}, payments: ${paymentCount}`);
}

// ─── SECTION 3.2 — Teacher-side exam results (for parent dashboard) ──────────
async function seedTeacherExams() {
  log('Section 3: exams + exam_subjects + exam_results (for parent Results screen)');
  ...
  await academicPool.query(
    `INSERT INTO exams (id, school_id, name, academic_year, start_date, end_date)
     VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (id) DO NOTHING`,
    [examId, SCHOOL_ID, 'Mid-Term 2026', '2025-26', '2026-09-01', '2026-09-15']
  ...
  const subjects = [
    { id: '88888888-8888-4888-a888-000000000011', classId: CLASS_1_ID, subject: 'Mathematics', teacher: TEACHER_1_ID, date: '2026-09-01' },
    { id: '88888888-8888-4888-a888-000000000012', classId: CLASS_2_ID, subject: 'English',     teacher: TEACHER_2_ID, date: '2026-09-03' },
  ];
  for (const s of subjects) {
    await academicPool.query(
      ...
      [s.id, examId, SCHOOL_ID, s.classId, s.subject, s.date, 100, 35, s.teacher, 'SUBMITTED']
